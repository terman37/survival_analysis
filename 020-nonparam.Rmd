...
```{r}
hist(dat$timeWeeks * 7 / 365.25) # scale in years
# carefull not meaningful as does not take care of censoring information
```
...
### Check censoring coding (0=censored, 1=not censored (event))

```{r}
with(dat,Surv(timeWeeks,delta))
```

43+ means censored data at week 43
...
Time in weeks might be cumbersome to read: we can re-express it in months
```{r}
dat <- mutate(dat, timeMonths = timeWeeks * 7 / 365.25*12)
fit.KM <- survfit(Surv(timeMonths, delta) ~ 1, data = dat, conf.type = "log-log") # conf.type : confidence interval
summary(fit.KM)
plot(fit.KM)
```
...
H0: Sa(t) = Sb(t)
large pvalue 0.7 cannot reject group0 = group1
...
pvalue:0.1 not significantly difference
...
pvalue < 0.05
Patch and combination treatment are significantly different
